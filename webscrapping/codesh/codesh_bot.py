...
class WebScrapping(webdriver.Chrome):
    ...
    def get_first_page_infos(self) -> list:
        container = self.find_element(By.CSS_SELECTOR, 'div[class="space-2 container"]')
        boxes = container.find_elements(By.CSS_SELECTOR, 'a[class="mb-5 card"]')
        companies = dict()
        for box in boxes:
            tag = box.find_element(By.TAG_NAME, 'a')
            name = tag.text
            stacks = box.find_element(By.CSS_SELECTOR, 'div[class="mb-3 mb-md-0"] > span[class="align-middle"]')
            ...
            if name not in companies.keys():
                ...
                companies[name]['stacks'] = list(set(companies[name]['stacks'] + [stack.text for stack in stacks]))
                companies[name]['vagas'].append(box.get_attribute('href'))
        return companies